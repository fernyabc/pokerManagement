...
struct ContentView: View {
    ...
    @StateObject private var streamService = WebRTCStreamCaptureService()
        ...
        TabView {
            NavigationView {
                ZStack {
                    ...
                    VStack(spacing: 16) {
                        statusBanner
                        PokerTableView(state: visionService.currentState).padding(.horizontal)
                        suggestionCard
                        Spacer()
                        Button(action: {
    ...
    private var statusBanner: some View {
        HStack {
            Circle().fill(streamService.isStreaming ? Color.green : Color.orange).frame(width: 10, height: 10)
            Text(streamService.connectionStatus).font(.subheadline).foregroundColor(streamService.isStreaming ? .green : .orange)
            Spacer()
        }
    ...
    private var suggestionCard: some View {
        VStack {
            if backendService.isFetching {
                ProgressView("Analyzing Board...").padding()
            } else if let suggestion = backendService.latestSuggestion {
                VStack(spacing: 8) {
                    HStack {
                        Text("GTO Output").font(.subheadline).foregroundColor(.secondary)
                        Spacer()
                        if let conf = suggestion.confidence {
                            Text("\(Int(conf * 100))% Freq").font(.caption).bold().padding(.horizontal, 8).padding(.vertical, 4).background(Color.blue.opacity(0.2)).cornerRadius(8)
                        }
                    }
                    Text(suggestion.action.uppercased()).font(.system(size: 38, weight: .black, design: .rounded)).foregroundColor(suggestion.action == "Raise" ? .red : .blue)
                    if let size = suggestion.raiseSize, suggestion.action == "Raise" {
                        Text("to $\(String(format: "%.1f", size))").font(.title2).bold()
                    }
                    if let reason = suggestion.reasoning {
                        Text(reason).font(.caption).italic().foregroundColor(.secondary).multilineTextAlignment(.center).padding(.top, 4)
                    }
                }
                .padding()
...
// Use the more comprehensive Equatable conformance
